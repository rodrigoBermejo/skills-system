...
Tabla extendida con ejemplos reales para cualquier proyecto.
...
|---|---|---|
| Construir workflow desde cero | `n8n` + `n8n-workflow-patterns` | "Crear el flow de bienvenida-calificacion para vertical-salud" |
| Configurar un nodo especifico | `n8n-node-config` | "Configurar el nodo Postgres para insertar con RETURNING" |
| Escribir logica en Code node | `n8n-code-javascript` | "Parsear la respuesta de OpenAI y extraer JSON" |
| Escribir expresiones en campos | `n8n-expressions` | "Acceder al body del webhook en un campo Set" |
| Validar antes de publicar | `n8n-validation` | "Revisar el workflow antes de deploy" |
| Buscar nodos o templates via MCP | `n8n-mcp-tools` | "Buscar si hay un nodo de WhatsApp Business" |
...
| Situacion real | Skill(s) | Ejemplo |
|---|---|---|
| Pagina Next.js nueva | `nextjs` | "Crear la pagina de settings del dashboard" |
| Endpoint Express nuevo | `express-mongodb` | "Agregar endpoint para listar contactos" |
| Componente UI | `frontend-design` | "Crear componente de tabla paginada" |
| Auth, JWT, permisos | `auth-security` | "Agregar role guard para admin" |
| Query PostgreSQL | `sql-databases` | "Implementar cursor pagination en conversaciones" |
| Integracion de pagos | `payment-integrations` | "Agregar webhook de pago exitoso" |
...
|---|---|
| Feature completo (plataforma) | `orchestrator` → `sdd-open-spec` → `sdlc-product-owner` → `sdlc-ux-designer` → skill de stack (nextjs, express-mongodb, etc.) → `sdlc-qa-engineer` → `sdlc-tech-lead` |
| Workflow N8N nuevo | `orchestrator` → `sdd-open-spec` → `n8n` → `n8n-workflow-patterns` → `n8n-code-javascript` → `n8n-validation` → `mcp-coordinator` |
...
| Audit de seguridad | `orchestrator` → `security-researcher` → `auth-security` → `sdlc-tech-lead` |
